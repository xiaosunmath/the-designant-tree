addLayer("a", {
    ...
    achievements: {
        ...
        41:{
            name: "生命？",
            done() {return getClickableState("c",6)}, 
            onComplete(){player.a.points=player.a.points.add(1)},
            tooltip: "购买碳原子", 
            textStyle: {'color': '#DD0000'},
        },
    },
    row: "side",
...
addLayer("e", {
    ...
    tabFormat: {
        "main": {
            ...
            ["display-text",
                function() {
                    if(player.e.points.gte("1e1000"))
                        return "你的员工数量超过了1e1000，引发了坍缩，超出部分被5次根"
                },
               {"color": "#FFFFFF", "font-size": "20px" }],"upgrades","blank","blank","buyables"],
        },
        "upgrades": {
...
addLayer("c", {
    ...
    quarkGain(){
        ...
        if(getClickableState("c",3)) gain = gain.mul(clickableEffect("c",3))
        return gain
    },
    atomGain(){
        let gain = player.c.quark.max(500000).div(500000).log(10).pow(2)
        if(hasUpgrade("c",31)) gain = gain.mul(upgradeEffect("c",31))
        if(getClickableState("c",2)) gain = gain.mul(clickableEffect("c",2))
        if(getClickableState("c",5)) gain = gain.mul(clickableEffect("c",5))
        return gain
    },
    update(diff){
    ...
    elementinfo(){
        ...
        for(let i = 1;i <= 118;i++){
            let disp = "[" + elementNames[i] + "]" + elementDisplay[i] + "<br>"
            if(!(layers.c.clickables[i].effect().gte(-1) && layers.c.clickables[i].effect().lte(-1))) disp = disp + "当前：x" + format(layers.c.clickables[i].effect()) + "<br>"
            disp = disp + "价格：" + format(layers.c.clickables[i].cost()) + "原子"
            eleinfo.push(disp)
        }
    ...
    tabFormat: {
        ...
        "the periodic table": {
            content: [
                ["display-text","目前作者只做到第六个元素",{"color": "#FFFFFF", "font-size": "15px" }],
                "blank",
                ["display-text",
                function() {
        ...
        "life": {
            content: [ ["infobox","lifeBox"],
            "blank","blank"],
            unlocked(){
                return getClickableState("c",6)
            },
        },
    },
    microtabs:{
    ...
    passiveGeneration(){
        if(getClickableState("c",4)) return 1
        else return 0
    },
    row: 2,
    hotkeys: [
...
const elementDatas = [null,
    {//1
        ccost(){return n(1e5)},
        effect(){return player.c.atom.add(1).log(10).pow(4)},
    },
    {//2
        ccost(){return n(2e5)},
        effect(){return player.d.points.add(1).root(2)},
    },
    {//3
        ccost(){return n(1e7)},
        effect(){return player.d.companypower.add(1).log(1.1)},
    },
    {//4
        ccost(){return n(5e7)},
        effect(){return n(-1)}
    ...
    {//5
        ccost(){return n(1e8)},
        effect(){return player.c.points.add(1).log(1.2)}
    },
    {//6
        ccost(){return n(1e10)},
        effect(){return n(-1)}
...
const elementDisplay = [null,
    "坍缩点获得基于原子倍增",
    "基于设计者倍增原子获得",
    "基于公司力量倍增夸克获得",
    "每秒获得100%的坍缩点",
    "基于坍缩点倍增原子",
    "碳基生命：解锁生命标签页(endgame)"
]
for (let i = 1; i <= 118; i++) {
    layers.c.clickables[i] = {
        title: elementNames[i],
        onClick() {
            if (player.c.eleshown == i) {
                if (this.canBuy() && getClickableState("c", i) == 0) setClickableState("c", i, 1)
            }
            player.c.eleshown = i
        },
        cost() {
            return elementDatas[i]?.ccost() ? elementDatas[i]?.ccost() : n("(e^1.79e308) 1")
        },
        canBuy() {
            return player.c.atom.gte(this.cost())
        },
        effect() {
            return elementDatas[i]?.effect() ? elementDatas[i]?.effect() : n("-1")
        },
        canClick() {
            return true
        },
        style() {
            let color = ""
            if (getClickableState("c", i) == 1) color = "#00BB00"
            else {
